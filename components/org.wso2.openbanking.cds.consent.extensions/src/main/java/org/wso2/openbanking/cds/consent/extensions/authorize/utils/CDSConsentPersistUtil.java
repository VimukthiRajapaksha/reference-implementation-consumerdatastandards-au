...
public class CDSConsentPersistUtil {
    ...
    public static void addNonPrimaryAccountDataToPersistData(
        ...
        for (Map.Entry<String, Map<String, List<String>>> entry : userIdNonPrimaryAccountsMap.entrySet()) {
            final String userId = entry.getKey();
            for (Map.Entry<String, List<String>> authTypeAndNonPrimaryAccount : entry.getValue().entrySet()) {
                ...
                if (currentUserIdNonPrimaryAccountsMap.containsKey(userId)) {
                    ...
                    currentUserIdNonPrimaryAccountsMap.put(userId, new HashMap<>(Map.of(authType, nonPrimaryAccounts)));
                }
            }
        }
    ...
    private static Map<String, Map<String, List<String>>> getUserIdAgainstAccountsMap(
        ...
        for (Map.Entry<String, Map<String, String>> entry : accountIdUserMap.entrySet()) {
            ...
            for (Map.Entry<String, String> userIdAndAuthType : entry.getValue().entrySet()) {
                ...
                if (userIdAccountsMap.containsKey(userId)) {
                    if (userIdAccountsMap.get(userId).containsKey(authType)) {
                        ...
                        userIdAccountsMap.get(userId).put(authType, new ArrayList<>(List.of(accountId)));
                    }
                } else {
                    userIdAccountsMap.put(userId, new HashMap<>(Map.of(authType, new ArrayList<>(List.of(accountId)))));
                }
            }
        }
